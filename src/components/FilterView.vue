<template>
  <div :class="{'open': isSort || isScreen}" @click.self="hideView">
    ...
    <section class="filter-extend" v-if="isSort">
      <ul>
        <li v-for="(item, index) in filterData.sortBy" :key="item.name" @click="selectSort(item, index)">
    ...
    <section class="filter-extend" v-if="isScreen">
      <div class="filter-sort">
        <div class="morefilter" v-for="screen in filterData.screenBy" :key="screen.id">
          <p class="title">{{screen.title}}</p>
          <ul>
            <li v-for="item in screen.data" :key="item.name" :class="{'selected': item.select}" @click="selectScreen(item, screen)">
